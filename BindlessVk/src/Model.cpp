...
void ModelSystem::LoadModel(const Model::CreateInfo& info)
{
	...
	BVK_ASSERT(!gltfContext.LoadASCIIFromFile(&input, &err, &warn, info.gltfPath), "Failed to load gltf file: {}@{}\nerr: {}", info.name, info.gltfPath, err);
	if (!warn.empty())
	{
	...
	/// Load material parameters
	{
		...
		{
			BVK_ASSERT(material.values.find("baseColorTexture") == material.values.end(),
