...
//  supported). 2022-09-01: Inputs: Honor GLFW_CURSOR_DISABLED by not setting mouse position.
...
#define GLFW_HAS_PER_MONITOR_DPI \
	(GLFW_VERSION_MAJOR * 1000 + GLFW_VERSION_MINOR * 100 >= 3300) // 3.3+
...
#define GLFW_HAS_OSX_WINDOW_POS_FIX                                                            \
	...
	#define GLFW_HAS_NEW_CURSORS \
		(GLFW_VERSION_MAJOR * 1000 + GLFW_VERSION_MINOR * 100 >= 3400) // 3.4+
...
#define GLFW_HAS_GAMEPAD_API \
	(GLFW_VERSION_MAJOR * 1000 + GLFW_VERSION_MINOR * 100 >= 3300) // 3.3+ glfwGetGamepadState() new
	                                                               // api
#define GLFW_HAS_GET_KEY_NAME \
	(GLFW_VERSION_MAJOR * 1000 + GLFW_VERSION_MINOR * 100 >= 3200) // 3.2+ glfwGetKeyName()
...
void ImGui_ImplGlfw_KeyCallback(GLFWwindow *window, int keycode, int scancode, int action, int mods)
{
	...
	io.SetKeyEventNativeData(imgui_key, keycode, scancode); // To support legacy indexing (<1.87
	                                                        // user code)
}
		...
		if (vd->WindowOwned) {
			...
			for (int i = 0; i < IM_ARRAYSIZE(bd->KeyOwnerWindows); i++)
				if (bd->KeyOwnerWindows[i] == vd->Window)
					ImGui_ImplGlfw_KeyCallback(vd->Window, i, 0, GLFW_RELEASE, 0); // Later params
					                                                               // are only used
					                                                               // for main
					                                                               // viewport, on
					                                                               // which this
