...
)
{
	...
	VkDeviceSize vertex_buffer_size_aligned = ((new_size - 1) / bd->BufferMemoryAlignment + 1)
...
)
{
	...
	ImGui_ImplVulkan_ViewportData *viewport_renderer_data = (ImGui_ImplVulkan_ViewportData *)
	                                                            draw_data->OwnerViewport
	                                                                ->RendererUserData;
...
void ImGui_ImplVulkan_Shutdown()
{
	...
	// for viewports
	ImGuiViewport *main_viewport = ImGui::GetMainViewport();
	if (ImGui_ImplVulkan_ViewportData *vd = (ImGui_ImplVulkan_ViewportData *)
...
)
{
	...
	// Create Swapchain
	{
		...
		info.imageSharingMode = VK_SHARING_MODE_EXCLUSIVE; // Assume that graphics family == present
		                                                   // family
...
)
{
	...
	for (int n = 0; n < platform_io.Viewports.Size; n++)
		if (ImGui_ImplVulkan_ViewportData *vd = (ImGui_ImplVulkan_ViewportData *)platform_io
		                                            .Viewports[n]
		                                            ->RendererUserData)
...
static void ImGui_ImplVulkan_CreateWindow(ImGuiViewport *viewport)
{
	...
	// single window, it halves framerate with a sec n / SDK 1.82.1)
...
static void ImGui_ImplVulkan_SwapBuffers(ImGuiViewport *viewport, void *)
{
	...
	wd->FrameIndex = (wd->FrameIndex + 1) % wd->ImageCount;         // This is for the next
	                                                                // vkWaitForFences()
	wd->SemaphoreIndex = (wd->SemaphoreIndex + 1) % wd->ImageCount; // Now we can use the next set
	                                                                // of semaphores
}
